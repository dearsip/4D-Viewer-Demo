...
public class FourDDemo : I4DSoftware
{
    ...
    public void Run(ref Vector3[] vertices, ref int[] triangles, double[][] rotate)
    {
        Vec.zero(reg1);
        ...
        convert(bufRelative, ref vertices, ref triangles);
    }
    ...
    private void drawFace(Geom.Shape shape, Geom.Face face)
    {
        ...
        Polygon polygon = new Polygon(vertex, Color.black);
        currentDraw.drawPolygon(polygon, Color.black, origin);
    }

    // Polygon の情報を vertices と triangles に変換する。
    private void convert(PolygonBuffer buf, ref Vector3[] vertices, ref int[] triangles)
    {
        int count = 0;
        verts.Clear();
        tris.Clear();
